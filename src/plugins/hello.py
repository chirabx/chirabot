...
hello_setu = on_message(rule=to_me(), block=False, priority=1)
matcher = on_command("来张涩图",rule=to_me(),aliases={"来张色图","来张图片","我要涩涩"},priority=5)
...
async def hello_handle(event: Event):
    ...
    # await hello.finish(f"不要说{message}，说来张涩图")  # 最终发送，完成后停止整个流程
